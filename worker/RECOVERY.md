...
- `partial` 表示仍有缺失；达到 `RANKING_MIN_COMPLETENESS_PERCENT=95` 即可发布当天成功仓库。
- `failed`、认证故障标记、用户取消阻止新版本发布，已发布版本保留。
- 完整度分母为本轮固定仓库集合，分子为集合中实际存在的当天 GitHub 快照；缺失项不入榜。
- 四周期榜单同一事务发布，补抓增加成功仓库后原子更新，每个版本自动发布最多尝试五次。
- 复查和隔离独立运行，少量 404/403 不阻止达到门槛的榜单发布。
- 页面展示发布时固化的完整度；历史榜单无记录时不推算。筛选不改变完整度口径。
