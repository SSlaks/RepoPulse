...
def finish_partial(job_key: str, message: str) -> dict[str, int | str]:
    ...
    with tasks.get_sync_session() as session:
        ...
        progress.update(publication="blocked", publication_reason="missing_daily_snapshots",
                        missing=sum(row.status != "saved" for row in rows),
    ...
    with tasks._task_lock("recovery-dispatch", timeout=240, required=True) as acquired:
        ...
        with tasks.get_sync_session() as session:
            jobs = session.scalars(select(JobRun).where(
                ...
                JobRun.status.in_(("running", "waiting", "completed")),
...
def _dispatch_job(session, job: JobRun, now: datetime) -> bool:
    ...
    if job.status == "completed":
        if progress.get("publication") == "pending":
    ...
    if utc(deadline) <= now:
        ...
        job.progress = dict(progress, publication="blocked", publication_reason="deadline")
        return False
